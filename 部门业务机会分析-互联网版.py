...
STAGE_WEIGHT = {
    '销售线索': 0.05, '公司立项': 0.10, '客户立项': 0.30,
    '技术认可': 0.50, '商务认可': 0.75, '签约准备': 1.00,
    '已赢单': 1.00, '已输单': 0.00
}
...
REQUIRED_COLS = ['阶段', '创建日期', '预计签约金额', '业务机会所有人', '销售类型',
                 '更新时间', '业务机会名', '项目类型', '可能性', '预计签约日期', '阶段修改日期']

ALL_STANDARD_NAMES = [
    '阶段', '预计签约金额', '业务机会所有人', '业务机会名', '创建日期', '销售类型',
    '更新时间', '项目类型', '可能性', '行业', '客户名', '销售部门', '产品描述',
    '预计签约日期', '未签报预计回款金额', '业务机会编号', '结果', '主要市场活动来源',
    '是否Name客户', '是否有定制开发', '已转签报单', '签约客户联系人', '最终客户',
    '最终客户联系人', '丢单原因', '竞争对手', '丢单原因备注', '价格手册', '客户编号',
    '预计供货日期', '是否预计签约当月验收', '阶段修改日期', '项目名称', '项目编号',
    '签报单流水号', '合同名称', '合同编号', '合计', '所有人编号', '所属公司',
    '原销售人员', '所有人部门', '创建人', '创建时间', '修改时间', '事业部',
]
...
class FunnelAnalyzer:
    ...
    def run_analysis(self):
        self.read_diagnosis = {'total_columns': len(self.df.columns), 'column_names': list(self.df.columns)}
    ...
    def _step0_filter_year(self):
        ...
        self.read_diagnosis['year_filter'] = {
            'before': total_before,
            'excluded': self.excluded_by_year,
            'after': after,
        }
    ...
    def _step1(self):
        ...
        for i, row in self.df.iterrows():
            ...
            if age > 365 * 3:
                reasons.append('ancient')
            stag_age = safe_timedelta(stage_mod_dt)
            if stag_age == 0:
                stag_age = safe_timedelta(update_dt)
            if stag_age == 0:
                stag_age = age
            if stage == '签约准备' and stag_age > 180:
                reasons.append('stagnant_prep')
            elif stage == '技术认可' and stag_age > 360:
                reasons.append('stagnant_tech')
            elif stage == '商务认可' and stag_age > 360:
                reasons.append('stagnant_comm')
            if amt == 0:
                reasons.append('zero_amount')
            if stage in ('商务认可', '签约准备'):
                try:
                    p = float(prob_str.replace('%', '')) if '%' in prob_str else float(prob_str)
                    if stage == '签约准备' and p < 90:
                        reasons.append('zero_prob')
                except:
                    pass
            if safe_timedelta(update_dt) > 180:
                reasons.append('no_update')
            if '纯借机' in proj_type:
                reasons.append('pure_borrow')
            if 0 < amt < 5000:
            ...
            for r in reasons:
                self.invalid_records.append({
                    'index': i,
                    'name': str(row.get('业务机会名', f'行{i}') or '')[:50],
                    'stage': stage,
                    'owner': str(row.get('业务机会所有人', '') or ''),
                    'amount': amt,
                    'create_date': create_dt,
                    'stage_mod_date': stage_mod_dt,
                    'reason_code': r,
                    'reason_text': INVALID_REASON_MAP.get(r, r),
                })
    ...
    def _step4(self):
        ...
        self.high_value = df.sort_values(
            ['_total_score', '_weighted_amt'], ascending=[False, False]
    ...
    def _step5(self):
        ...
        self.summary = {
            ...
            'high_value_amount': hva,
            'high_value_weighted': hvw,
            'stage_distribution': sd,
            'owner_distribution': od,
            'missing_cols': self.missing_cols,
            'read_diagnosis': self.read_diagnosis,
...
def generate_report(analyzer, chart_files, output_path):
    ...
    if analyzer.missing_cols:
        ...
        wp2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    r = dp2.add_run(
        f'📊 当前分析：{diag.get("total_columns", 0)}列，'
    ...
    for item in ['一、数据总览与核心指标', '二、漏斗健康度诊断', '三、无效业务机会识别与剔除',
                 '四、有效业务机会甄别与高价值机会清单', '五、人员漏斗透视', '六、管理行动建议']:
        p = doc.add_paragraph()
        r = p.add_run(item)
    ...
    if si:
        for issue in si:
            ...
            if '⚠️' in issue:
                r.font.color.rgb = RGBColor(231, 76, 60)
    else:
    ...
    for code, records in sorted(rg.items(), key=lambda x: -len(x[1])):
        ...
        for rec in records[:50]:
            ...
            inv_d.append([
                str(row.get('业务机会名', ''))[:35], str(rec['stage']), str(rec['owner']),
                format_amount(rec['amount']), format_date(row.get('预计签约日期', pd.NaT)),
                format_date(rec.get('stage_mod_date', pd.NaT)),
            ])
        add_styled_table(doc, ['业务机会名', '阶段', '负责人', '金额', '预计签约', '阶段修改'],
                         inv_d, [5, 1.8, 1.5, 2, 2, 2])
        if len(records) > 50:
            p = doc.add_paragraph()
    ...
    for run in p.runs:
        set_run_font(run, font_name='宋体', size=Pt(10))
    if not analyzer.high_value.empty:
        for cff in chart_files:
    ...
    for rank, (_, row) in enumerate(analyzer.high_value.iterrows(), 1):
        if '_is_direct' in row.index and row['_is_direct']:
            st_val = '⭐' + str(row.get('销售类型', ''))
        else:
            st_val = str(row.get('销售类型', ''))
        hv_d.append([
            str(rank), str(row.get('业务机会名', ''))[:30], str(row.get('阶段', '')),
            str(row.get('业务机会所有人', '')), st_val, format_amount(row.get('_amt', 0)),
            format_amount(row.get('_weighted_amt', 0)), f'{row.get("_total_score", 0):.0f}',
            format_date(row.get('创建日期', pd.NaT)), str(row.get('行业', '')),
        ])
    add_styled_table(doc, ['排名', '业务机会名', '阶段', '负责人', '销售类型', '金额', '加权', '评分', '创建', '行业'],
                     hv_d[:80], [1, 4.5, 1.5, 1.5, 1.5, 1.8, 1.8, 0.8, 1.8, 2])
    if len(hv_d) > 80:
    ...
    if ow:
        ...
        for n in sorted(ow.keys(), key=lambda x: -ow[x]['amount']):
            ...
            if ratio < 25:
                ...
                risk = '🟢 健康'
            od_rows.append([n, str(v['count']), format_amount(v['amount']),
                           format_amount(v['weighted']), f'{ratio:.1f}%', risk])
        add_styled_table(doc, ['负责人', '有效机会', '名义金额', '加权金额', '折算率', '风险提示'],
                         od_rows, [2, 2, 3, 3, 2, 6])
    doc.add_page_break()
    ...
    for run in h2.runs:
        set_run_font(run, font_name='宋体')
    for a in [
        f'1. 跟踪{analyzer.summary["high_value_count"]}个高价值机会'
        f'(加权{format_amount(analyzer.summary["high_value_weighted"])})。',
        '2. Top10指派售前资源。',
        '3. 周会有效业务机会检查。',
        '4. ⭐直签项目优先。',
    ]:
...
# ================================================================
def main():
    st.set_page_config(page_title='销售漏斗智能分析工具 v1.0', page_icon='📊', layout='wide')

    st.title('📊 销售漏斗智能分析工具 v1.0')
    st.caption('筛选2026年签约 ｜ 有效业务机会五步甄别 ｜ 高价值机会清单 ｜ 人员漏斗透视 ｜ 管理行动建议')
    ...
    if uploaded_file is not None:
        run_btn = st.button('🚀 开始分析并生成报告', type='primary', use_container_width=True)
    else:
        st.info('👈 请在左侧上传业务机会Excel文件')
        run_btn = False
    ...
    if run_btn and uploaded_file is not None:
        ...
        try:
            ...
            _log('📖 纯pandas读取Excel...')
            try:
                df, header_idx = smart_read_excel(tmp_path)
            except Exception as e:
                _log(f'❌ 读取失败: {e}')
                st.error(f'读取失败：{e}')
                return
            _log(f' ✅ 表头行: 第{header_idx + 1}行')
            _log(f' ✅ {len(df)}条记录, {len(df.columns)}个字段')
            cols_display = list(df.columns[:8])
            if len(df.columns) > 8:
                cols_display.append('...')
            _log(f' 📋 列名: {", ".join(cols_display)}')
            found = [c for c in REQUIRED_COLS if c in df.columns]
            missing = [c for c in REQUIRED_COLS if c not in df.columns]
            if found:
                _log(f' ✅ 已识别({len(found)}/{len(REQUIRED_COLS)}): {", ".join(found)}')
            if missing:
                _log(f' ⚠️ 缺失({len(missing)}/{len(REQUIRED_COLS)}): {", ".join(missing)}')
            ...
            if cm:
                for old, new in cm.items():
                    df[new] = df[old]
                _log(f' ✅ 映射: {", ".join(f"{k}→{v}" for k, v in cm.items())}')
            else:
                _log(' ✅ 列名已匹配')
            found2 = [c for c in REQUIRED_COLS if c in df.columns]
            _log(f' 📋 最终识别: {len(found2)}/{len(REQUIRED_COLS)}')
            for c in ['阶段', '预计签约金额', '业务机会所有人', '业务机会名', '预计签约日期', '阶段修改日期']:
                if c in df.columns:
                    sample = df[c].dropna().head(3).tolist()
                    _log(f' 📋 [{c}] {sample}')

            _log('⚙️ 第0步：筛选2026年签约项目...')
            _log('⚙️ 运行分析引擎...')
            analyzer = FunnelAnalyzer(df)
            summary = analyzer.run_analysis()
            yf = summary.get('read_diagnosis', {}).get('year_filter', {})
            if yf:
                _log(f' 📅 原始{yf.get("before",0)}条 → 排除{yf.get("excluded",0)}条非2026年 → 保留{yf.get("after",0)}条')
            _log(f' 总:{summary["total_count"]} 无效:{summary["invalid_count"]}'
                 f' 有效:{summary["valid_count"]} 高价值:{summary["high_value_count"]}')
            _log(f' 总金额:{format_amount(summary["total_amount"])}'
                 f' 有效金额:{format_amount(summary["valid_amount"])}'
                 f' 加权金额:{format_amount(summary["high_value_weighted"])}')
            if analyzer.missing_cols:
            ...
            _log(f' ✅ {len(charts)}张图表')

            _log('📝 生成DOCX报告...')
            ts = datetime.now().strftime('%Y%m%d_%H%M')
            rn = f'销售漏斗分析报告_{ts}.docx'
            rp = os.path.join(output_dir, rn)
